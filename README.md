...
<br>
<center>
<img src='kernel-trick-equation.jpg'>
...
Pretty nifty, huh? The equation above says that we should start couting at 0, because we see `n=1` under the summation. We should stop once we've hit 5 (inclusive). Our sum total the sum of each value of X for each step between `n=1` and up to (and including) `n=5`. 
...
Another common type of notation is **_Pi Notation_**, which works similarly to Sigma Notation. The only real difference here is that Pi Notation mutliplies each step, rather than adds them to a total. With our understanding of Sigma Notation, it isn't that hard to figure out an example of Pi Notation. 
...
As you can see from the example above, there isn't too much of a difference between sigma an pi notation. The only difference is that with Sigma, you add the results of each step, whereas with Pi, you multiply them!
